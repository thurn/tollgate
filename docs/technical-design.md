...
| `tg push-master [--wait]` | Discover the clean, linear local `master` range after certified `release`, authorize each commit oldest-first, and let configured remote synchronization push the certified chain; optionally wait for the tail. |
...
State-changing commands accept a command UUID for idempotency internally; a CLI retry after lost IPC response cannot duplicate an approval or cancel.

### 16.2 Output and exit behavior

`tg approve` returns after durable enqueue rather than waiting 10+ minutes. `--wait` is equivalent to enqueue followed by `tg wait`. Read commands have stable versioned `--json`; human output may evolve. JSON uses full IDs/OIDs and enum values, while human output may show unambiguous prefixes.
